...
public:
	...
	void update(const double target, const double endS = 0.0) {
		...
		// V^2-Vo^2 =2as;
			...
			double ut = constrain(myTimer.getTime(), 0.00, upTime);
			double mt = constrain(myTimer.getTime() - upTime, 0.00, maxPowerTime);
			double dt = constrain(myTimer.getTime() - (upTime + maxPowerTime), 0.00, downTime);
			double targets = acc * sq(ut) * 0.50 + startSpeed * ut
				+ maxSpeed * mt
				+ (-acc * sq(dt) * 0.50) + limitSpeed * dt;

			if (!nextState) {//経路を巡行してないときだけ更新する
				nextNowSpeed = startSpeed + acc * ut + -acc * dt;
				nextNowPos = targets + nowPos;
			}
			if (upTime + downTime + maxPowerTime <= myTimer.getTime()) {
